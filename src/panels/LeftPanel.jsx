...
import ExportManager from '../components/ExportManager.jsx'; // <--- 1. IMPORT QO'SHING
...
function LeftPanel() {
    ...
    const handleUrlLoad = () => {
        if (urlInput.trim()) loadImage(urlInput.trim());
    };
    ...
    return (
        <div>
            ...
            <div className="mb-6 border border-gray-700 rounded-md p-4">
                <div className="flex justify-between items-center mb-3">
                    ...
                    <div className="flex space-x-3">
                        <button onClick={() => setShowNewClassForm(!showNewClassForm)} title="Add new class">
                            <FontAwesomeIcon icon={faPlus} className="text-blue-400 hover:text-blue-300"/></button>
                        <button onClick={() => setIsEditingClasses(!isEditingClasses)}
                                title={isEditingClasses ? "Finish Editing" : "Edit Classes"}><FontAwesomeIcon
                            icon={faPencilAlt}
                            className={isEditingClasses ? 'text-yellow-400' : 'text-gray-400 hover:text-white'}/>
                        </button>
                    </div>
                </div>
                <div className="space-y-2">
                    ...
                    {objectClasses?.map((cls) => (
                        <div key={cls.name}
                             className={`flex items-center p-2 border rounded-md cursor-pointer transition ${selectedClass?.name === cls.name ? 'selected-class' : 'border-transparent'}`}
                             style={{
                                 backgroundColor: selectedClass?.name === cls.name ? '' : `${cls.color}33`,
                                 borderColor: selectedClass?.name === cls.name ? '' : cls.color,
                            ...
                            <div className="flex-1 flex items-center justify-between">
                                ...
                                <span
                                    className="text-xs text-gray-500 bg-gray-700/50 px-1.5 py-0.5 rounded-md">{cls.tool}</span>
                            </div>
                            <FontAwesomeIcon icon={cls.icon} className="ml-3 mr-3 text-gray-400"/>
                            {isEditingClasses && (<button onClick={(e) => {
                                e.stopPropagation();
                                handleDeleteClass(cls.name);
                            }} className="text-red-500 hover:text-red-400" title={`Delete ${cls.name}`}><FontAwesomeIcon
                                icon={faTrash}/></button>)}
                        </div>
                    ))}
                </div>
                ...
                {showNewClassForm && (
                    <div className="mt-4 bg-gray-800/50 p-3 rounded-md border border-gray-700">
                        ...
                        <input type="text" placeholder="New class name"
                               className="w-full border border-gray-600 rounded-md px-2 py-1.5 text-sm bg-gray-700 text-gray-300 mb-2"
                        ...
                        <div className="mb-3">
                            ...
                            <select
                                className="w-full border border-gray-600 rounded-md px-2 py-1.5 text-sm bg-gray-700 text-gray-300"
                                value={newClassToolType} onChange={(e) => setNewClassToolType(e.target.value)}>
                                {availableToolIcons?.map(tool => (
                                    <option key={tool.type} value={tool.type}>{tool.name}</option>))}
                            </select>
                        </div>
                        <button onClick={handleAddClass}
                                className="w-full bg-blue-600 hover:bg-blue-700 text-white py-1.5 rounded-md transition">Add
                            Class
                        </button>
                    </div>
                )}
            ...
            <div className="border border-gray-700 rounded-md p-4">
                ...
                <div
                    className="border-2 border-dashed border-gray-600 rounded-lg p-4 mb-3 text-center transition hover:bg-gray-700/50">
                    <label htmlFor="fileInput" className="cursor-pointer flex flex-col items-center">
                        <FontAwesomeIcon icon={faCloudUploadAlt}
                                         className="text-4xl text-blue-500 mb-2 hover:text-blue-400"/>
                        <p className="text-gray-400 text-sm">Drag & Drop or <span
                            className="underline">Upload Image</span></p>
                ...
                <div className="flex items-center gap-2">
                    ...
                    <input type="text" placeholder="https://..."
                           className="flex-1 border border-gray-600 rounded-md px-2 py-1.5 text-sm bg-gray-700 text-gray-300"
                           value={urlInput} onChange={(e) => setUrlInput(e.target.value)} onKeyDown={handleKeyDown}/>
                    <button onClick={handleUrlLoad}
                            className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-md text-sm transition">Load
                    </button>
                </div>
